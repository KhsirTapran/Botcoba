...
module.exports = devil = async (devil, mek) => {
	try {
        ...
        const listmsg = (from, title, desc, list) => { // ngeread nya pake rowsId, jadi command nya ga keliatan
            let po = devil.prepareMessageFromContent(from, {"listMessage": {"title": title,"description": desc,"buttonText": "☕MENU BOT","footerText": "SILAHKAN PILIH LIST MENU\nDI BAWAH","listType": "SINGLE_SELECT","sections": list}}, {})
      ...
      const promoteAdmin = async function(to, target=[]){
           ...
           reply(`Hm..... @${mentioned[0].split('@')[0]} JABATANMU DINAIKAN TIDAK MENJADI BUDAK LAGI`)
}
}
}
      const demoteAdmin = async function(to, target=[]){
           ...
           reply(`Hm..... @${mentioned[0].split('@')[0]} JABATANMU DITURUNKAN LEBIH RENDAH DARI BUDAK :)`)
...
function banChat() {
    ...
    if (new Date() * 1 - settingstatus > 1000) {
      ...
      await devil.setStatus(`Aktif Selama: ${uptime} | Mode: ${banChats ? 'SELF-MODE' : 'PRIVATE-MODE'}`).catch((_) => _);
...
➸ *Sejak* : ${getTime}`
      mentions(cptl, x, true)
		...
		if (afk.checkAfkUser(sender, _afk) && !isCmd) {
		    ...
		    const pep = `*${pushname}* telah kembali dari AFK! Selamat datang kembali Beban tim~`
		    reply(pep)
 ...
 case 'donasi':
               txtt =`Hai Kak.....\n*${pushname}*\n MAU DONASI LEWAT APA, KITA MODE PRIVATE JADI TIDAK PERLU DONASI GAPAPA YA KAK :)`

               buttons = [{buttonId: '!QRIS',buttonText:{displayText: 'NoPayments'},type:1},{buttonId:'!gopay',buttonText:{displayText:'NoPayments'},type:1},{buttonId:'!pulsa',buttonText:{displayText:'NoPayments'},type:1}]

               buttonsMessage = {
               contentText: `${txtt}`,
               footerText: 'GAUSAH DONASI BRO, BOT PRIVATE',
               buttons: buttons,
               headerType: 1
}
...
┃⬡𝘽𝙊𝙏 𝙈𝙊𝘿𝙀 : ${banChats ? "𝑺𝑬𝑳𝑭-𝑴𝑶𝑫𝑬" : "𝐏𝐑𝐈𝐕𝐀𝐓𝐄 - 𝐌𝐎𝐃𝐄"}
┃⬡𝙎𝙋𝙀𝙀𝘿 : ${latensie.toFixed(4)} _Second
┃⬡𝙁𝙄𝙏𝙐𝙍 : 𝑳𝒊𝒉𝒂𝒕 𝒔𝒆𝒏𝒅𝒊𝒓𝒊 𝒅𝒊 𝒃𝒂𝒘𝒂𝒉
┗⬣

┏━━⬣ 𝐈𝐍𝐅𝐎 𝐔𝐒𝐄𝐑
┃⬡ NAMA : *${pushname}*
┃⬡ STATUS : *${isOwner ? 'OWNER' : isPremium ? 'Premium' : 'Member'}*
...
`, imageMessage: imageMsg,
        ...
        case 'command':
               list = []
               listmenu = [`groupmenu`,`stickermenu`,`makermenu`,`ownermenu`,`funmenu`,`infomenu`]
               listmenuu = [`Menu Group`,`Sticker Menu`,`Maker Menu`,`Fun Menu`,`Info Menu`]
               nombor = 1
               startnum = 0
               for (let x of listmenu) {
        ...
        case 'aljxixnu':
        case 'menuuu':
               const boy = "19788381056@s.whatsapp.net"     
...
┃⬡ 𝑩𝒐𝒕 𝑴𝒐𝒅𝒆 : ${banChats ? "𝑺𝑬𝑳𝑭-𝑴𝑶𝑫𝑬" : "𝗣𝗿𝗶𝘃𝗮𝘁𝗲-𝑴𝑶𝑫𝑬"}
┃⬡ 𝑭𝒊𝒕𝒖𝒓 : Upcoing 𝒇𝒊𝒕𝒖𝒓
┗⬣
...
//------------------< Store jualan >-------------------              
         case 'sgsuejre':
               ...
               listmenuu = [`Servicesan`,`Sampingan`,`JASA EDIT`,`MURID BOT`,`JASA HITUNG POINT FT`,`ROOM CARD FF`,`WEB JUALAN`,`CULIK MEMBER`,`SUNTIK FOLL IG`]
               nombor = 1
               startnum = 0
               for (let x of listmenu) {
               const yy = {title: 'silahkan pilih menunya ' + nombor++,
...
case 'buttonstik':
              gopeynya = 'https://i.ibb.co/RYNmnbf/ganteng.jpg'
...
buttons = [{buttonId: `${prefix + command}`,buttonText:{displayText: `➡️Next`},type:1},{buttonId:`${prefix}nhentaibot`,buttonText:{displayText:'NHENTAI BOT'},type:1}]
              ...
              buttonsMessage = {footerText:'Ngopi dulu ges sambil ngudud ☕', imageMessage: imageMsg,
...
//------------------< Ingfo Bot >-------------------
      case 'runtime':
              ...
              break
       case 'youtube': 
              reply(`Napa buka YT bro?, nih link Youtube:\n https://youtube.com`)
              break
       case 'masukandata':
...
//------------------< Fun >-------------------
    case 'public':
       ...
       case 'cekwatak':
              ...
              const watak = ['top deh pokoknya','Kang 5 Jari','Predator Bocil','Sangean','penyayang','pemurah','Pemarah','Pemaaf','Penurut','Baik','baperan','Baik-Hati','penyabar','UwU','Suka Membantu']
...
//------------------< Menunya Bang:v >-------------------
      case 'infoig':
             reply(`Ini Instagram OWNERKU :) : https://www.instagram.com/onlykape`)
             break
      case 'grupbot':
             reply('')
             break
